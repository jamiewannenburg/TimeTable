...
subprocess.call("python setup_win.py install py2exe")

# get new version number
p = os.path.relpath('binaries/windows')
filename = 'TimeTable_v0-001.zip'
if os.path.exists(os.path.join(p,'TimeTable_v0-001.zip')):
    count = 0
    for f in os.listdir(p):
        if re.search(r'TimeTable_v0-\d{3}.zip',f).group(0):
            candidate = int(re.search(r'\d{3}',f).group(0))
            if candidate > count:
                count = candidate
            
    filename = 'TimeTable_v0-%.3d.zip'% (count+1)
...
# make setup and run py2app install (installs to /dist)
shutil.rmtree('dist')
shutil.rmtree('build')
os.remove('setup.py')
subprocess.call("python script_py2applet.py --make-setup TimeTable.py")
subprocess.call("python setup.py install py2app")

# get new version number
p = os.path.relpath('binaries/mac')
filename = 'TimeTable_v0-001.zip'
if os.path.exists(os.path.join(p,'TimeTable_v0-001.zip')):
    count = 0
    for f in os.listdir(p):
